    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(testloader):
            ...
            if batch_idx % 10 == 0:  # TODO fix
                print('Epoch: %d, Loss: %.3f | Acc: %.3f%% (%d/%d)' % (epoch_number, test_loss / (batch_idx + 1),
...
def train_single_epoch(net, trainloader, epoch_number, device, criterion, optimizer, lr_scheduler,
                       clip_gradient, grad_clip_cst, lsr,  accum_steps, print_batch_stat_freq, outF, batch_size,  world_size):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        inputs, targets = inputs.to(device), targets.to(device) # pass the data to the GPU for anything except mSAM

        # Run single step of training

        outputs, loss = train_vanilla_single_step(net=net, inputs=inputs, targets=targets,
                                                    criterion=criterion, accum_steps=accum_steps,
                                                    trainloader=trainloader, batch_idx=batch_idx,
                                                    optimizer=optimizer, lr_scheduler=lr_scheduler,
                                                    clip_gradient=clip_gradient, grad_clip_cst=grad_clip_cst, lsr=lsr)
        ...
        if batch_idx % print_batch_stat_freq == 0:
            ...
            print('Epoch: %d, Batch: %d, Loss: %.3f | Acc: %.3f%% (%d/%d)' % (
                epoch_number, batch_idx, train_loss * accum_steps / (batch_idx + 1), 100. * correct / total, correct, total))

    # Print epoch-end stats
    acc = 100. * correct / total
    print(
        "For epoch number: {}, train loss: {} and accuracy: {}".format(epoch_number, train_loss * accum_steps / (batch_idx + 1), acc))
    outF.write(
        "For epoch number: {}, train loss: {} and accuracy: {}".format(epoch_number, train_loss * accum_steps / (batch_idx + 1), acc))
    outF.write("\n")
    outF.flush()
...
def train_vanilla_single_step(net, inputs, targets, criterion, accum_steps, trainloader, batch_idx, optimizer,
                              lr_scheduler, clip_gradient, grad_clip_cst, lsr):
...
def main_trainer(rank, world_size, args, use_cuda):
    ...
    train_loader, test_loader = get_train_and_test_dataloader(dataset=args.dataset, batch_size=args.batch_size,
    ...
    if args.model == 'resnet18':
        ...
        net = Wide_ResNet(depth=28, widen_factor=10, dropout_rate=0.0,
                          num_classes=args.num_classes if not args.pretrained else 100)  # TODO introduce a parameter for dropout
    else:
    ...
    for epoch in range(args.num_epochs):
        ...
        train_single_epoch(net=net, trainloader=train_loader, epoch_number=epoch, device=device, criterion=criterion,
                           optimizer=optimizer, lr_scheduler=lr_scheduler, clip_gradient=args.clip_gradient, grad_clip_cst=args.grad_clip_cst,
                           lsr=args.lsr, accum_steps=args.accum_steps,
                           print_batch_stat_freq=args.print_batch_stat_freq, outF=outF,  batch_size = args.batch_size,
                                                 world_size = world_size)
        # Compute test accuracy
        test_acc, test_loss = compute_test_stats(net=net, testloader=test_loader, epoch_number=epoch, device=device,
                                                 criterion=criterion, outF=outF)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default='cifar10', type=str, help='type of dataset (cifar10 or cifar100)')
    parser.add_argument('--batch_size', default=1000, type=int, help='batch size (per gpu) for training sets. The effective batch size is num_gpu*batch_size')

    # Model arguments
    parser.add_argument('--model', default='resnet18', type=str, choices=['resnet18', 'resnet50', 'WRN-28-10'],
    ...
    parser.add_argument('--lr_schedule_type', default='onecycle', type=str, choices=['onecycle'],
                        help='type of learning rate scheduler')
    parser.add_argument('--classifier_lr', default=0.8, type=float, help='learning rate for classification layer')
    ...
    parser.add_argument('--clip_gradient', type=str2bool, nargs='?', const=True, default=True,
                        help="clips the gradient.")
    parser.add_argument('--grad_clip_cst', type=float, help='constant of gradient clipping')
    # Training arguments
    parser.add_argument('--adaptive_lr', type=str2bool, nargs='?', const=True, default=False,
                        help="whether to divide lr by clipping constant.")
    parser.add_argument('--linear_probing', type=str2bool, nargs='?', const=True, default=False,
                        help="finetune last-layer only.")
    parser.add_argument('--num_epochs', default=200, type=int, help='number of epochs')
    parser.add_argument('--accum_steps', default=1, type=int, help='number of gradient accumulation steps (not supported by mSAM)')
    parser.add_argument('--print_batch_stat_freq', default=1, type=int,
    ...
    parser.add_argument('--use_gn', type=str2bool, nargs='?', const=True, default=True,
                        help="uses opacus validator to change the batch norms by group normalization layers.")

    # Logging arguments
    parser.add_argument('--out_file', default='output_file.txt', type=str, help='output file for logging')
    parser.add_argument('--out_bare', default='output_bare.txt', type=str, help='output file for logging')
    parser.add_argument('--save_file', default='output_net.pt', type=str, help='output file for saving the network')
    # Random seed
    ...
    print(use_cuda)
    args.out_bare = str(args.dataset)+'_'+str(args.model)+'_'+str(args.optimizer)+'_'+str(args.seed)+'.txt'
    print(args.out_bare)
    ...
    if world_size > 1: # multiple gpus available
        mp.spawn(main_trainer, args=(world_size, args, use_cuda), nprocs=world_size, join=True)
    else: # single gpu
