...
model,data_loader,test_loader = model_factory(args.model, dset_path, True, args.seed, args.nsamples, batch_size=args.batchsize)
...
if args.lintune:
    ...
    for i in range(args.lintune_epochs):
        ...
        for batch in data_loader:
            ...
            optim.step()
            optim.zero_grad()
        print('%02d %.4f' % (i, cumloss / len(data_loader)))
...
if args.gap:
    ...
    for i in range(args.gap_epochs):
        ...
        print('%05d: %.6f' % (i, cumloss / len(data_loader)))
...
acc = test(modelp, test_loader)
name = './results/%s' % (args.load)
name_save_model = './results/%s' % (args.load.replace(".txt",".pth"))
