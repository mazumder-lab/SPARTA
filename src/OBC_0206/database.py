...
class SparsityDatabase:
    ...
    def load_errors(self, name):
        path = os.path.join(self.prefix, "scores/%s_%s_%s.txt" % (self.model, self.sparsetype, name))
    ...
    def get_params(self, layers):
        ...
        for name in layers:
            ...
            for sparsity in self.sds:
                res[name][sparsity] = torch.sum((self.sds[sparsity][name + ".weight"] != 0).float()).item()
        return res
    ...
    def get_flops(self, layers, model, sample, run):
        ...
        for name in layers:
            ...
            for sparsity in self.sds:
                res[name][sparsity] = (
                    flops[name] * torch.mean((self.sds[sparsity][name + ".weight"] != 0).float()).item()
                )
        return res
    ...
    def get_sparsity(self, layers, model, sample, run):
        ...
        for name in layers:
            ...
            for sparsity in self.sds:
                res[name][sparsity] = (
                    sparsitys[name] * torch.mean((self.sds[sparsity][name + ".weight"] != 0).float()).item()
                )
        return res
...
if __name__ == "__main__":
    ...
    dset_paths = {"imagenet": IMAGENET_PATH, "cifar10": CIFAR10_PATH, "mnist": MNIST_PATH}

    dset_path = dset_paths[args.dataset]

    model, data_loader, test_loader = model_factory(
        args.model, dset_path, True, args.seed, args.nsamples, batch_size=args.batchsize
    )
    ...
    # with torch.no_grad():
    #    print(run(modelp, next(iter(data_loader)), loss=True) / args.nsamples)
    # test(modelp, test_loader)
    # exit()
        ...
        def accumerrs(name):
            def tmp(layer, inp, out):
                ...
                for config in sorted(db.sds):
                    ...
                    errs[name][config] = (
                        errs[name].get(config, 0) + torch.sum((layersp[name](inp[0].data) - out.data) ** 2).item()
                    )
    ...
    if args.mode == "loss":
        ...
        baseloss /= len(data_loader) * args.nrounds
        for name in errs:
            for config in errs[name]:
                errs[name][config] /= len(data_loader) * args.nrounds
