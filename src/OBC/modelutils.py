...
def test_yolo(model, dataloader):
    ...
    anno_json = dataloader.dataset.original.path.replace("images/val2017", "annotations/instances_val2017.json")
    import random
...
def get_yolo(var):
    ...
    model.hyp = {"box": 0.05, "cls": 0.5, "cls_pw": 1.0, "obj": 1.0, "obj_pw": 1.0, "fl_gamma": 0.0, "anchor_t": 4}
    model = model.to(DEV)
...
def firstlast_names(model):
    ...
    if "bertsquad" in model:
        return ["bert.embeddings.word_embeddings", "bert.embeddings.token_type_embeddings", "qa_outputs"]
    if "yolo" in model:
