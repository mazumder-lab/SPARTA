...
def random_subset(train_dataset, nsamples, seed):
    if nsamples != -1:
        ...
        indices_to_remove, indices_train_subsample, targets_to_remove, targets_train_subsample = train_test_split(
            indices_train, targets_train, test_size=nsamples, stratify=targets_train, random_state=0
        )
...
def imagenet_get_datasets_old(data_dir):
    ...
    train_transform = [transforms.RandomResizedCrop(224), transforms.RandomHorizontalFlip()]
...
def model_factory(
    arch, dset_path, pretrained=True, seed=0, nsamples=-1, initialize_bn=True, batch_size=128, name_dataset=None
):
    if arch == "resnet50":
        ...
        state_trained = torch.load(
            "../network_pruning/checkpoints/resnet20_cifar10.pth.tar", map_location=torch.device("cpu")
        )["state_dict"]
