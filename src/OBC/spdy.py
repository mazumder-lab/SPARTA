...
parser.add_argument(
    "--constr",
    choices=["", "bits", "bops", "flops", "timingsq", "sparsity"],
    default="",
)
...
if args.score:
    ...
    print(
        ...
        / (
            baseline_constr
            - prunabletime
            + sum(t[s] for s, t in zip(solution, timings)) * bucketsize
        )
    )
...
for resamplings in range(int(0.1 * len(layers)), 0, -1):
