...
from utils_dataset import read_batch, update_dataset
from tqdm import tqdm
from utils_training import Pos_embed
...
def get_data(test_pass_first_modules = True):
    global name_dataset, seed, n_train_kept, num_workers, batch_size_dataset, rel_damp
...
#%%
for arch in l_arch:
    loader_train, loader_val, loader_test = get_data(test_pass_first_modules=False)
    original_model, criterion, modules_to_prune = get_model(arch, seed, pretrained=True, with_z=False, gamma=1.0, prune_bias=False, activation_fn="relu")
    original_model.to(device)
    pruned_model, criterion, modules_to_prune = get_model(arch, seed, pretrained=True, with_z=False, gamma=1.0, prune_bias=False, activation_fn="relu")
    pruned_model.to(device)

    first_modules = torch.nn.Sequential(*[original_model.patch_embed, Pos_embed(original_model), original_model.patch_drop, original_model.norm_pre])
    update_dataset(first_modules, device, first_modules, loader_train, None, n_train_kept, False, None, None, 3)
    ...
    for goal_sparisty in l_sparsity:
        ...
        pruned_model.load_state_dict(torch.load(path_convex_non_uniform, map_location=device))
        n_blocks = len(pruned_model.blocks)
        for ind_block in range(n_blocks):
            import ipdb;ipdb.set_trace()
            pruned_model
