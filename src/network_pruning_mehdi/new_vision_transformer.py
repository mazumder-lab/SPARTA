...
from timm.layers import PatchEmbed, DropPath, RmsNorm, PatchDropout, SwiGLUPacked, \
    trunc_normal_, lecun_normal_, resample_patch_embed, resample_abs_pos_embed, use_fused_attn, \
    get_act_layer, get_norm_layer, LayerType
from new_mlp import Mlp
from new_attention_pool import AttentionPoolLatent
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        if with_z:
            self.qkv = Linear_with_z(dim, dim * 3, bias=qkv_bias, gamma=gamma)
        else:
        ...
        if with_z:
            self.proj = Linear_with_z(dim, dim, gamma=gamma)
        else:
