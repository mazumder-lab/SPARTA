...
then
    ...
    echo "Saving machine name $master_addr..."
    echo $master_addr > "machine_name.txt"
    echo "done"
else
...
#echo "torchrun" --nnodes=$3 --nproc_per_node=1 --rdzv_id=434 --rdzv_backend=c10d --rdzv_endpoint=$MASTER_ADDR:$MASTER_PORT main.py --lr 0.005 --arch mobilenetv1 --name_dataset imagenet --optimizer_name SGD --selection_reg 0.1 --l2_reg 1e-04 --pretrained True --goal_sparsity 0.9 --folder_saves Saves_combined_pretrained_mobilenet_test --num_workers 20 --test_distributed 1 --type_reset ensemble --type_pruning magnitude --mode layer_wise --loss_func layer_wise --node_rank $1
#torchrun --nnodes=$3 --nproc_per_node=1 --rdzv_id=434 --rdzv_backend=c10d --rdzv_endpoint=$MASTER_ADDR:$MASTER_PORT main.py --lr 0.005 --arch mobilenetv1 --name_dataset imagenet --optimizer_name SGD --selection_reg 0.1 --l2_reg 1e-04 --pretrained True --goal_sparsity 0.9 --folder_saves Saves_combined_pretrained_mobilenet_test --num_workers 0 --test_distributed 1 --type_reset ensemble --type_pruning magnitude --mode layer_wise --loss_func layer_wise --node_rank $1
/home/gridsan/gafriat/.conda/envs/pruning/bin/torchrun --nnodes=$3 --nproc_per_node=1 --rdzv_backend=c10d --rdzv_endpoint=$MASTER_ADDR:$MASTER_PORT main.py --lr 0.005 --arch resnet50 --name_dataset imagenet --optimizer_name SGD --selection_reg 0.1 --l2_reg 1e-04 --pretrained True --goal_sparsity 0.9 --folder_saves Saves_test_DDP --num_workers 0 --test_distributed 1 --type_reset ensemble --type_pruning magnitude --mode layer_wise --loss_func layer_wise --node_rank $1