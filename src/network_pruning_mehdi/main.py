...
parser.add_argument('--test_start_obc', type=int, default = 0,
                    help='wether to prune first using obc and then fine tune the weights using layer-wise and/or actual loss function')
...
parser.add_argument('--test_start_convex', type=int, default = 0,
                    help='wether to prune first using sparse-gpt and then fine tune the weights using layer-wise and/or actual loss function')
parser.add_argument('--algo_pruning', type=str, default = "mp_convex",
                    help='which algorithm to use, add _convex in the convex case')
...
parser.add_argument('--test_almost_sequential', type=int, default = 0,
...
parser.add_argument('--test_save_all_sparsities', type=int, default = 0 ,
                    help='whether to save the weights for different sparsities in the OBC_convex case')
parser.add_argument('--n_parallel', type=int, default = 128,
                    help='number of layers to prune in parallel')
...
parser.add_argument('--test_save_losses_and_Traces', type=int, default = 0,
                    help='whether to save all the Losses and Traces from the potential pruning of each weight')
...
if __name__ == '__main__':
    ...
    test_start_obc = arguments.test_start_obc
    rel_damp = arguments.rel_damp
    lambda_fisher = arguments.lambda_fisher
    lambda_reconst = arguments.lambda_reconst
    algo_pruning = arguments.algo_pruning
    test_save_all_sparsities = arguments.test_save_all_sparsities
    n_parallel = arguments.n_parallel
    n_convex = arguments.n_convex
    pruning_level = arguments.pruning_level
    n_layers = arguments.n_layers
    test_save_losses_and_Traces = arguments.test_save_losses_and_Traces

    if goal_sparsity_discrete == -1 and (test_start_convex or test_start_obc or test_start_sparse_gpt):
        goal_sparsity_discrete = goal_sparsity
    ...
    name_dataset_paths = {'imagenet':IMAGENET_PATH,'cifar10':CIFAR10_PATH, 'cifar100':CIFAR100_PATH,
    ...
    test_update_test_vit = False
    
    train_val_dataset, test_dataset = get_dataset(name_dataset, name_dataset_path, n_train_kept, get_item_func, arch, seed, activation_fn, device, test_almost_sequential, test_update_test_vit)
        ...
        def objective(trial):
            ...
            for ind_repeat in range(n_repeat):
                ...
                if loss_func == "layer_wise" or l2_original_reg!=0.0:
                    ...
                    if test_save_all_sparsities:
                        #n_sparsities = 5
                        #sparsities = list(np.linspace(goal_sparsity - (goal_sparsity)/2, goal_sparsity, n_sparsities)) + list(np.linspace(goal_sparsity, goal_sparsity+(1-goal_sparsity)/2, n_sparsities))[1:]
                        # sparsities = np.linspace(0, 0.975, 40)
                        sparsities = list(np.linspace(0, 0.8, 33))+list(np.linspace(0.8, 0.99, 20))[1:]
                        sparsities = [round(x, 5) for x in sparsities]
                        # default_sparsities = (0, .99, .1)
                        # sparsities = []
                        # density = 1 - default_sparsities[0]
                        # while density > 1 - default_sparsities[1]:
                        #     sparsities.append(1 - density)
                        #     density *= 1 - default_sparsities[2]
                        # sparsities.append(default_sparsities[1]) # Change this to the sparsity levels I want!
                        sds = {s: copy.deepcopy(original_model).cpu().state_dict() for s in sparsities}
                        if n_convex != -1:
                            d_losses = {s: copy.deepcopy(original_model).cpu().state_dict() for s in sparsities}
                        else:
                            d_losses = None
                    else:
                        sds = None
                        d_losses = None
                        sparsities = [goal_sparsity]
                ...
                if loss_func == "layer_wise" and mode == "layer_wise":
                    ...
                    # set_require_grad_rec(model, False)
                    if "facebook/opt" in arch:
                        l_modules_before_threshold = get_modules_layer_wise_llm(model, train_val_attention_mask, test_one_layer_pruning)
                        l_modules_before_threshold_original = get_modules_layer_wise_llm(original_model, train_val_attention_mask, test_one_layer_pruning)
                    elif "deit" in arch:
                    ...
                    if ("resnet18" == arch):
                        if test_one_layer_pruning:
                            l_modules_before_threshold = l_modules_before_threshold[:-1]+[(Avgpool_2d_resnet18(),0,0,0)]+[(View_resnet(),0,0,0)]+l_modules_before_threshold[-1:]
                            l_modules_before_threshold_original = l_modules_before_threshold_original[:-1]+[(Avgpool_2d_resnet18(),0,0,0)]+[(View_resnet(),0,0,0)]+l_modules_before_threshold_original[-1:]
                        else:
                            l_modules_before_threshold = l_modules_before_threshold[:-1]+[Avgpool_2d_resnet18()]+[View_resnet()]+l_modules_before_threshold[-1:]
                            l_modules_before_threshold_original = l_modules_before_threshold_original[:-1]+[Avgpool_2d_resnet18()]+[View_resnet()]+l_modules_before_threshold_original[-1:]
                    ...
                    if (test_one_layer_pruning and not("opt" in arch)) or not(test_one_layer_pruning):
                        ...
                        l_modules, l_modules_original = l_modules_before_threshold, l_modules_before_threshold_original

                    if "facebook/opt" in arch:
                        for module in l_modules:
                            module.seqlen = model.seqlen
                            if test_one_layer_pruning:
                                set_phase_decoder_rec(module, 1)
                            else:
                                set_phase_decoder_rec(module, 0)
                            module.test_save_initial_data = False
                            module.test_add_initial_data = False
                            module.test_downsample = False
                    ...
                    d_params_temp = dict(model.named_parameters())
                    for name_param in d_params_temp:
                        d_params_temp[name_param].name_in_model = name_param.replace(".weight", "")
                                        
                    acc_downsamples = 0
                    for ind_model_wrap in range(len(l_model_wrap)):
                        print(f"ind_model_wrap: {ind_model_wrap+1}/{len(l_model_wrap)}")
                        ...
                        if test_prop_goal_sparsity:
                            ...
                            goal_sparsity_sub_module = goal_sparsity_used
                        if is_last_module and (name_dataset in ["c4", "wikitext2", "ptb"] or "deit" in arch):# and (len(list(model_wrap.model.children())) <= 1 or test_one_layer_pruning):
                        ...
                        #     goal_sparsity_sub_module = 0.0
                        retraining_of_last_block = False
                        if is_last_module and batch_size_dataset == 16 and name_dataset in ["c4", "wikitext2", "ptb"]:
                    ...
                    set_require_grad_rec(main_model_wrapper.model, True)
                    final_sparsity = main_model_wrapper.get_final_sparsity()
                    set_require_grad_rec(main_model_wrapper.model, False)
                    ...
                    if test_save_losses_and_Traces:
                        import pickle
                        with open(f'gpts_{arch}_{end_name}.pickle', 'wb') as handle:
                            pickle.dump(gpts, handle, protocol=pickle.HIGHEST_PROTOCOL)

                    if test_save_all_sparsities:
                        
                        folder_saves_unstr = f"Saves_OBC_{end_name}"

                        if not(os.path.exists(folder_saves_unstr)):
                            os.mkdir(folder_saves_unstr)

                        if not(os.path.exists(f"{folder_saves_unstr}/models_unstr")):
                            os.mkdir(f"{folder_saves_unstr}/models_unstr")

                        if n_convex != -1:
                            if not(os.path.exists(f"{folder_saves_unstr}/losses")):
                                os.mkdir(f"{folder_saves_unstr}/losses")

                        for sparsity in sparsities:
                            name = '%s_%04d.pth' % (arch, int(sparsity * 10000))
                            torch.save(sds[sparsity], os.path.join(f"{folder_saves_unstr}/models_unstr", name))
                            if n_convex != -1:
                                torch.save(d_losses[sparsity], os.path.join(f"{folder_saves_unstr}/losses", name))
                    ...
                    # Process the results
                    if name_dataset in ["c4", "wikitext2", "ptb"] or "deit" in arch:
                ...
                save_results(dict_list=dict_list, best_model=best_model, d_results=d_results, name_study=name_study, dict_params=dict_params, trial=trial, folder_saves=folder_saves, ind_repeat=ind_repeat, sds=sds)
