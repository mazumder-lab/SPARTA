...
l_name_datasets = ["cifar10", "cifar100"]
                            ...
                            for epochs in l_epochs:
                                for (classifier_lr, lr) in l_lrs:
                                    for name_dataset in l_name_datasets:
                                        ...
                                        command = f'{supercloud_environment} train_cifar.py --use_fixed_w_mask_finding {use_fixed_w_mask_finding} --use_delta_weight_optim {use_delta_weight_optim} --use_w_tilde True --correction_coefficient 0.01 --dataset {name_dataset} --batch_size {batch_size} --model {model} --num_classes {num_classes} --classifier_lr {classifier_lr} --lr {lr} --lsr 0.0 --wd 0.0 --momentum 0.9 --lr_schedule_type "onecycle" --num_epochs {epochs} --magnitude_descending False --warm_up 0.01 --finetune_strategy "all_layers"   --use_gn True --method_name "optim_averaged_clipped_grads"   --mask_type "optimization" --sparsity {sparsity} --epsilon {epsilon} --delta 1e-5 --clipping {clipping} --experiment_dir {EXPERIMENT_DIR} --out_file "optim_averaged_clipped_grads.txt"  --seed 0  --SLURM_JOB_ID 0 --TASK_ID 0'
