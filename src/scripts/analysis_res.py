...
l_columns = ["dataset", "model", "batch_size", "sparsity", "finetune_strategy", "epsilon", "delta", "use_fixed_w_mask_finding", "use_delta_weight_optim", "use_w_tilde", "classifier_lr", "lr", "clipping", "Train acc", "Test acc", "Finished epochs"]
    ...
    for dataset in l_dataset:
        for finetune_strategy in l_finetune_strategy:
            ...
            for sparsity in l_sparsity_actual:
                condition = (res_tot["model"]==model)*(res_tot["dataset"]==dataset)*(res_tot["finetune_strategy"]==finetune_strategy)
                to_add = ""
                if test_sparsity:
                    condition = condition*(res_tot["sparsity"]==sparsity)
                    to_add += f"_{int(100*sparsity)}"
                res_model = res_tot[condition]
                res_model[l_columns].sort_values(by="Test acc", ascending=False).to_csv(f"""{path_save}/res_{model.replace("'","")}_{dataset.replace("'","")}_{finetune_strategy.replace("'","")}{to_add}.csv""")
                try:
...
df_best_res[l_columns+["Finished exp"]].sort_values(by="Test acc", ascending=False).to_csv(f"""res_global/res_global{type_method}.csv""")

# %%
df_clipped_grad = pd.read_csv(f"""res_global/res_global_clipped_grads.csv""")
df_row_pruning = pd.read_csv(f"""res_global/res_global_row_pruning_noisy_grads.csv""")
...
for model in l_models:
    for dataset in l_dataset:
        ...
        for type_method in ["_clipped_grads", "_row_pruning_noisy_grads"]:
            ...
            l_sparsity = np.array(df_best_res[(df_best_res["dataset"]==dataset)*(df_best_res["model"]==model)]["sparsity"])
            l_test_acc = np.array(df_best_res[(df_best_res["dataset"]==dataset)*(df_best_res["model"]==model)]["Test acc"])
