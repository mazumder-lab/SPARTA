...
for (
    ...
    print(
        "seed,fisher_size, num_stages,num_iterations,first_order_term,sparsity,l2,sparsity_schedule,algo,block_size",
        seed,
        fisher_size,
        num_stages,
        num_iterations,
    ...
    if seed != old_seed:
        ...
        model, train_dataset, test_dataset, criterion, modules_to_prune = model_factory(
            arch, dset_path, pretrained=pretrained
        )
        ...
        ##########
        recompute_bn_stats = (
            arch == "resnet20" or arch == "resnet18"
        )  # Resnet20 models need updating their BN statistics
    ...
    prun_dataloader = DataLoader(
        train_dataset,
        batch_size=fisher_mini_bsz,
        shuffle=True,
        num_workers=num_workers,
        pin_memory=True,
    )
    ...
    acc_different_methods.append(
        {
            ...
            "runtime": end - start,
            "seed": seed,
            "fisher_mini_bsz": fisher_mini_bsz,
            "fisher_subsample_size": fisher_subsample_size,
            "num_iterations": num_iterations,
            "num_stages": num_stages,
            "recompute_bn_stats": recompute_bn_stats,
            "ignore_bias": True,
    ...
    torch.save(model_to_save.state_dict(), f"chita_model_{int(args.sparsity * 100)}_{l2}.pth")
