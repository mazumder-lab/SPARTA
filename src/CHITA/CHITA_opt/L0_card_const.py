...
def prox_L1(beta, beta_tilde1, lambda1):
    ...
    return beta_tilde1 + np.where(abs_beta > lambda1, abs_beta - lambda1, 0) * np.sign(
        beta_sub
    )
...
def hard_thresholding(
    y, X, beta, r, k, alpha, lambda1, lambda2, beta_tilde1, beta_tilde2, L, M=np.inf
):

    beta_hat = (1 - 2 * lambda2 / L) * beta + (
        X.T @ r - alpha + 2 * lambda2 * beta_tilde2
    ) / L
    beta_new = clip(prox_L1(beta_hat, beta_tilde1, lambda1 / L), M)
    rec_obj = (
        (1 / 2) * (beta_new - beta_hat) ** 2
        + lambda1 / L * np.abs(beta_new - beta_tilde1)
        - ((1 / 2) * (beta_hat) ** 2 + lambda1 / L * np.abs(beta_tilde1))
    )
...
def hard_thresholding_ls(
    ...
    L_change = np.min(
        np.where(
            same_sign + (np.abs(grad[support]) < max_suppinv),
            np.abs(beta[support])
            / (np.abs(grad[support]) * (2 * same_sign - 1) + max_suppinv),
            np.inf,
        )
    )
    ...
    opt_step = (grad_supp @ alpha - r @ Xgrad - 2 * lambda2 * beta_sub2 @ grad_supp) / (
        Xgrad @ Xgrad + 2 * lambda2 * grad_supp @ grad_supp
    )
    ...
    if opt_step < L_change:
        ...
        f_best = evaluate_obj(
            beta, r, alpha, lambda1, lambda2, beta_tilde1, beta_tilde2
        )
        sea_itr = 0
        ...
        while sea_itr < sea_max_itr:
            ...
            f_new = evaluate_obj(
                beta_tmp, r_tmp, alpha, lambda1, lambda2, beta_tilde1, beta_tilde2
            )
...
def prune_ls(
    ...
    opt_step = (
        (grad_supp @ alpha) - (r @ Xgrad) - 2 * lambda2 * (beta_sub2 @ grad_supp)
    ) / ((Xgrad @ Xgrad) + 2 * lambda2 * (grad_supp @ grad_supp))
    ...
    if sup_max >= supinv_max - 1e-10:
        ...
        f_best = evaluate_obj(
            beta_mp, y - X @ beta_mp, alpha, lambda1, lambda2, beta_tilde1, beta_tilde2
        )
        sea_itr = 0
        while sea_itr < sea_max_itr:
...
@nb.njit(cache=True)
def DIHT_update(
    y, X, r, gamma, k, alpha, lambda1, lambda2, beta_tilde1, beta_tilde2, eta, M=np.inf
):
    ...
    rec_obj = (
        (1 / 2) * quad_term * beta**2
        - linear_term * beta
        + lambda1 * np.abs(beta - beta_tilde1)
        - lambda1 * np.abs(beta_tilde1)
    )
    argsort = np.argsort(rec_obj)
    beta[argsort[k:]] = 0
    ...
    for i in range(sto_iter):
        ...
        if sto_m == "cyc":
            for j in range(len(stepL)):
                ...
                beta_hat = (1 - 2 * lambda2 / L) * beta_cur + (
                    X[:, gradB[j] : gradB[j + 1]].T @ r
                    - alpha[gradB[j] : gradB[j + 1]]
                    + 2 * lambda2 * beta_tilde2[gradB[j] : gradB[j + 1]]
                ) / L
                beta_new = clip(
                    prox_L1(
                        beta_hat, beta_tilde1[gradB[j] : gradB[j + 1]], lambda1 / L
                    ),
                    M,
                )
...
def coordinate_descent(
    ...
    beta_inew = clip(
        prox_L1(
            2 * lambda2 * beta_tilde2[i] + X[:, i] @ r + S_diag[i] * beta[i] - alpha[i],
            beta_tilde1[i] * (S_diag[i] + 2 * lambda2),
            lambda1,
        )
        / (S_diag[i] + 2 * lambda2),
        M,
...
def Check_Swap1(
    ...
    linear_term = (
        2 * lambda2 * beta_tilde2[j]
        + X[:, j] @ r
        + (X[:, j] @ X[:, i]) * beta[i]
        - alpha[j]
    )
    beta_jnew = clip(
        prox_L1(linear_term, quad_term * beta_tilde1[j], lambda1) / (quad_term), M
    )
    beta_reduce = (
        (1 / 2) * quad_term * beta_jnew**2
        - linear_term * beta_jnew
        + lambda1 * (np.abs(beta_jnew - beta_tilde1[j]) - np.abs(beta_tilde1[j]))
    )
...
def Check_Swaprow(
    ...
    bst_imp, bst_i, bst_v, f = (
        0,
        -1,
        0,
        evaluate_obj(beta, r, alpha, lambda1, lambda2, beta_tilde1, beta_tilde2),
    )
    for i in support:
        beta_jnew, beta_reduce = Check_Swap1(
            y,
            X,
            beta,
            r,
        ...
        cur_imp = (
            evaluate_obj(beta, r, alpha, lambda1, lambda2, beta_tilde1, beta_tilde2) - f
        )
        beta[i] = beta_iold
        beta[j] = 0
        r = r_old
...
@nb.njit(cache=True)
def Check_Swapall(
    y, X, beta, r, S_diag, alpha, lambda1, lambda2, beta_tilde1, beta_tilde2, M=np.inf
):
    ...
    for j in support_c:
        bst_imp, bst_i, bst_v = Check_Swaprow(
            y,
            X,
            beta,
            r,
...
def evaluate_obj(beta, r, alpha, lambda1, lambda2, beta_tilde1, beta_tilde2):
    ...
    return (
        0.5 * (r @ r)
        + lambda2 * (beta_sub2 @ beta_sub2)
        + lambda1 * (np.sum(np.abs(beta_sub1)))
        + alpha @ beta
    )
...
def initial_active_set(
    ...
    for i in range(act_itr):
        beta_tmp, r_tmp = hard_thresholding(
            y,
            X,
            beta_tmp,
            r_tmp,
    ...
    while diht_cur_itr < diht_max_itr:
        ...
        if diht_cur_itr % 100 == 0:
            ...
            print(
                ...
                (X.T @ gamma) @ beta
                + alpha @ beta
                + lambda2 * (beta - beta_tilde2) @ (beta - beta_tilde2)
                + lambda1 * np.sum(np.abs(beta - beta_tilde1))
                - (1 / 2) * gamma @ gamma
                - gamma @ y,
            )
...
def Vanilla_SDIHTCD(
    ...
    while diht_cur_itr < diht_max_itr:
        ...
        for batch_iter in range(len(idx_list) - 1):
            beta, gamma, Xgamma, nnz_idx = SDIHT_update(
                y,
                X,
                X[idx_list[batch_iter] : idx_list[batch_iter + 1], :],
                beta,
                gamma,
                Xgamma,
                idx_list[batch_iter],
                idx_list[batch_iter + 1],
                nnz_idx,
                k,
                alpha,
                lambda1,
...
def Active_SDIHTCD(
    ...
    while act_cur_itr < act_max_itr:
        ...
        lambda2_other = (
            lambda2 * beta_tilde2 @ beta_tilde2
            - lambda2 * beta_tilde2[active_set] @ beta_tilde2[active_set]
        )
        lambda1_other = lambda1 * np.sum(np.abs(beta_tilde1)) - lambda1 * np.sum(
            np.abs(beta_tilde1[active_set])
        )

        while diht_cur_itr < diht_max_itr:

            for batch_iter in range(len(idx_list) - 1):
                beta_act, gamma, Xgamma_act, nnz_idx = SDIHT_update(
                    y,
                    X_act,
                    X_act[idx_list[batch_iter] : idx_list[batch_iter + 1], :],
                    beta_act,
                    gamma,
                    Xgamma_act,
                    idx_list[batch_iter],
                    ...
                    lambda1,
                    lambda2,
                    beta_tilde1[active_set],
                    beta_tilde2[active_set],
                    eta,
                    M,
                )
            ...
            if diht_cur_itr % 100 == 0:
                ...
                print(
                    "dual   obj ",
                    (X_act.T @ gamma) @ beta_act
                    + alpha[active_set] @ beta_act
                    + lambda2
                    * (beta_act - beta_tilde2[active_set])
                    @ (beta_act - beta_tilde2[active_set])
                    + lambda2_other
                    + lambda1_other
                    + lambda1 * np.sum(np.abs(beta_act - beta_tilde1[active_set]))
                    - (1 / 2) * gamma @ gamma
                    - gamma @ y,
                )
        ...
        sols.append(
            Solution(
                ...
                objs=objs,
                r=np.copy(r),
                diht_cur_itr=diht_cur_itr,
                sol_time=sol_time,
                search_itr=search_cur_itr,
...
def Vanilla_IHTCD(
    ...
    iht_cur_itr = 0
    objs = []
    beta, r = hard_thresholding(
        y, X, beta, r, k, alpha, lambda1, lambda2, beta_tilde1, beta_tilde2, L, M
    )
    f_old = evaluate_obj(beta, r, alpha, lambda1, lambda2, beta_tilde1, beta_tilde2)
    while iht_cur_itr < iht_max_itr:
        ...
        while True:
            ...
            if (
                f_new < f_old - L_init / 10 * (beta_tmp - beta) @ (beta_tmp - beta)
                or search_cur_itr == 0
            ):
                beta_update = beta_tmp
                r_update = r_tmp
...
def Active_IHTCD(
    ...
    while act_cur_itr < act_max_itr:
        ...
        beta_act, f, objs, r_act, iht_cur_itr, sol_time = Vanilla_IHTCD(
            y,
            X_act,
            beta_act,
            k,
...
def Active_IHTCDLS(
    ...
    while act_cur_itr < act_max_itr:
        ...
        beta_act, f, objs, r_act, iht_cur_itr, sol_time = Vanilla_IHTCDLS(
            y,
            X_act,
            beta_act,
            k,
...
def Active_IHTCDPSI(
    ...
    while psi_cur_itr < psi_max_itr:
        ...
        psi_cur_itr += 1
        j, bst_i, bst_v = Check_Swapall(
            y, X, beta, r, S_diag, alpha, lambda1, lambda2, beta_tilde1, beta_tilde2, M
        )
        if j == -1:
            break
        else:
...
def Stochastic_BCD(
    ...
    for i in range(len(gradB) - 1):
        stepL.append(
            1.05 * (skl_svd(X_act[:, gradB[i] : gradB[i + 1]]) ** 2 + lambda2**2)
        )

    beta_act, r, objs = sto_BCD(
        y,
        X_act,
        beta_act,
        alpha[act_idx],
...
def compute_inverse(y, X, alpha, lambda2, beta_tilde2, act_idx=None):
    ...
    if n < k:
        if act_idx is None:
            ...
            beta = solve_b / (2 * lambda2) - X.T @ np.linalg.solve(
                np.eye(n) + X @ (X.T) / (2 * lambda2), X @ solve_b
            ) / (4 * lambda2**2)
        else:
            solve_b = (
                pmvm(X, y, act_idx, True)
                + 2 * lambda2 * beta_tilde2[act_idx]
                - alpha[act_idx]
            )
            mmX = pmmm(X, act_idx)
            mvmX = pmvm(X, solve_b, act_idx, False)
            solve_tmp = (
                np.linalg.solve(np.eye(n) + mmX / (2 * lambda2), mvmX)
                / (4 * lambda2**2)
            ).astype(X.dtype)
            beta = solve_b / (2 * lambda2) - pmvm(X, solve_tmp, act_idx, True)
    else:
        if act_idx is None:
            solve_b = X.T @ y + 2 * lambda2 * beta_tilde2 - alpha
...
def Heuristic_LS(
    ...
    if p < 1e7:
        beta_act = beta[act_idx]
        X_act = X[:, act_idx]
        beta_act = compute_inverse(
            y, X_act, alpha[act_idx], lambda2, beta_tilde2[act_idx]
        )
    else:
...
def Vanilla_IHTCD_PP(
    ...
    beta, f, objs, r, iht_cur_itr, sol_time = Vanilla_IHTCD(
        y,
        X[:, nnz_idx],
        beta[nnz_idx],
        k,
        ...
        lambda1=lambda1,
        lambda2=lambda2,
        beta_tilde1=beta_tilde1[nnz_idx],
        beta_tilde2=beta_tilde2[nnz_idx],
        L=L,
        M=M,
        iht_max_itr=iht_max_itr,
        ftol=ftol,
        cd_itr=cd_itr,
        ctol=ctol,
        search_max_itr=search_max_itr,
    )
...
def Active_IHTCD_PP(
    ...
    beta, f, sols, r, act_cur_itr, tot_time = Active_IHTCD(
        y,
        X[:, nnz_idx],
        beta[nnz_idx],
        k,
        ...
        iht_max_itr=iht_max_itr,
        ftol=ftol,
        act_max_itr=act_max_itr,
        buget=buget,
        kimp=kimp,
        act_itr=act_itr,
        cd_itr=cd_itr,
        ctol=ctol,
        sea1_max_itr=sea1_max_itr,
        sea2_max_itr=sea2_max_itr,
    )
...
def Active_IHTCDLS_PP(
    ...
    if len(nnz_idx) > k:
        beta, f, sols, r, act_cur_itr, tot_time = Active_IHTCDLS(
            y,
            X[:, nnz_idx],
            beta[nnz_idx],
            k,
...
def Heuristic_CD_PP(
    ...
    if len(nnz_idx) > k:
        ...
        beta, f, r, sol_time = Heuristic_CD(
            y,
            X[:, nnz_idx],
            beta[nnz_idx],
            k,
