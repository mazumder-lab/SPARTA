...
def cosine_lr(epochs, max_lr, min_lr):
    return [
        min_lr + 0.5 * (max_lr - min_lr) * (1 + np.cos(np.pi * epoch / epochs))
        for epoch in range(epochs)
    ]


def cosine_lr_restarts(
    epochs,
    max_lr,
    min_lr,
    n_prune_epochs,
    ...
    if gamma_ft < 0:
        lr_list += cosine_lr(
            epochs - prune_every * (n_prune_epochs - 1), ft_max_lr, ft_min_lr
        )
    else:
        lr_list += [
            ft_max_lr * np.power(gamma_ft, i)
            for i in range(epochs - prune_every * (n_prune_epochs - 1))
        ]
    return lr_list
...
def weight_rewinding_mobilenetv1(epochs):
    ...
    max_lr = 0.256
    min_lr = 0
    lr_list = [max_lr] * warmup + cosine_lr_restarts(
        epochs - warmup, max_lr, min_lr, 1, epochs - warmup
    )
    momentum = 0.875
