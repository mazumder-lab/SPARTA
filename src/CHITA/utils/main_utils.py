...
from CHITA_opt.L0_card_const import (
    Active_IHTCDLS_PP,
    Heuristic_CD_PP,
    Heuristic_LS,
    Heuristic_LSBlock,
...
from models_mehdi.resnet import ResNet18
...
def get_blocklist(model, params, block_size):
    ...
    for p in params:
        ...
        if param_size < block_size:
            ...
            block_subdiag = list(
                range(i_w, i_w + param_size + 1, int(param_size / num_block))
            )
...
def set_pvec(w, model, params, device, nhwc=False):
    ...
    for p in params:
        ...
        if type(w) == torch.Tensor:
            ...
            state_dict[p] = (
                torch.Tensor(w[i : (i + count)]).to(device).reshape(state_dict[p].shape)
            )
        i += count
...
def get_gvec(model, params):
    ...
    return torch.cat([named_parameters[p].grad.reshape(-1) for p in params])
...
def get_gvec1(model, params):
    ...
    return torch.cat(
        [
            named_parameters[p].grad_sample.reshape(
                named_parameters[p].grad_sample.shape[0], -1
            )
            for p in params
        ],
        dim=1,
...
def generate_schedule(num_stages, base_level, sparsity_level, schedule):
    ...
    if schedule == "exp":
        sparsity_multiplier = (
            (sparsity_level - base_level)
            * np.power(2, num_stages - 1)
            / (np.power(2, num_stages - 1) - 1)
        )
        l = [
            base_level
            + sparsity_multiplier * ((np.power(2, stage) - 1) / np.power(2, stage))
        ...
        l = [
            sparsity_level
            + (base_level - sparsity_level)
            * np.power(1 - (stage / (num_stages - 1)), 3)
            for stage in range(num_stages)
        ]
        return [x for x in l for _ in range(repeat)]
    elif schedule == "const":
        ...
        return [
            base_level + stage * (sparsity_level - base_level) / (num_stages - 1)
            for stage in range(num_stages)
        ]
    elif schedule == "MFAC":
        sparsity_multiplier = ((1.0 - sparsity_level) / (1.0 - base_level)) ** (
            1.0 / num_stages
        )
        return [
            1.0 - ((1.0 - base_level) * (sparsity_multiplier ** (stage + 1)))
            for stage in range(num_stages)
        ]
...
def mnist_get_datasets(data_dir):
    ...
    test_dataset = datasets.MNIST(root=data_dir, train=False, transform=test_transform)
...
def model_factory(arch, dset_path, pretrained=True):
    if arch == "resnet18":
        ...
        state_trained = torch.load(
            "checkpoints/mnist_25_epoch_93.97.ckpt", map_location=torch.device("cpu")
        )["model_state_dict"]
        new_state_trained = OrderedDict()
        for k in state_trained:
            if "mask" in k:
                continue
            new_state_trained[k.split(".")[1] + "." + k.split(".")[3]] = state_trained[
                k
            ]
        if pretrained:
        ...
        state_trained = torch.load(
            "checkpoints/resnet20_cifar10.pth.tar", map_location=torch.device("cpu")
        )["state_dict"]
        new_state_trained = OrderedDict()
        for k in state_trained:
        ...
        train_dataset = datasets.CIFAR10(
            root=dset_path, train=True, download=True, transform=train_transform
        )
        test_dataset = datasets.CIFAR10(
            root=dset_path, train=False, download=True, transform=test_transform
        )
        ...
        if pretrained:
            ...
            state_trained = torch.load(path, map_location=torch.device("cpu"))[
                "state_dict"
            ]
