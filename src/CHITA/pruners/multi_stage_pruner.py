...
class MultiStagePruner:
    ...
    def prune(self, mask0, sparsity, base_level, grads=None, FILE=None):
        sparsities = generate_schedule(
            self.num_stages, base_level, sparsity, self.schedule
        )
        ...
        for i, sparsity_stg in enumerate(sparsities):
            ...
            if not self.test_dataloader is None:
                ...
                self.results[-1]["test_acc"] = compute_acc(
                    self.pruner.model, self.test_dataloader, self.pruner.device
                )
