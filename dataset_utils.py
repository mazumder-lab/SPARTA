...
def get_training_dataloader(mean, std, batch_size=16, num_workers=1, world_size = 1, rank = 0, shuffle = True, ViT = False):
    ...
    cifar100_training = torchvision.datasets.CIFAR100(root='../datasets', train=True, download=True,
                                                      transform=transform_train)
    if world_size > 1: # Create a parallel sampler for multi-gpu case
        sampler = DistributedSampler(cifar100_training,
                                 num_replicas=world_size,
                                 rank=rank,
                                 shuffle=shuffle)
        cifar100_training_loader = torch.utils.data.DataLoader(
            cifar100_training, shuffle=False, sampler = sampler, num_workers=num_workers, batch_size=batch_size, pin_memory=False)
    else:
        cifar100_training_loader = torch.utils.data.DataLoader(
...
def get_train_and_test_dataloader(dataset="cifar10", batch_size=1000, world_size = 1, rank = 0, shuffle = True, ViT = False, test_batch_size = 128):
    if dataset == "cifar100":
        ...
        if world_size > 1: # Create a parallel sampler for multi-gpu case
            ...
            cifar10_training_loader = torch.utils.data.DataLoader(
                trainset, batch_size=batch_size, shuffle=False, num_workers=1 ,sampler = sampler, pin_memory=False)
        else:
            cifar10_training_loader = torch.utils.data.DataLoader(
